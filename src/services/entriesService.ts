...
export async function getAvailableMonthKeys(householdId: string, fallbackCount = 18): Promise<string[]> {
  ...
  if (!householdId) {
    logWithTs("getAvailableMonthKeys: missing householdId, using fallback", { fallbackCount });
    return base;
  }

  logWithTs("getAvailableMonthKeys: loading months from Firestore", { householdId, fallbackCount });
  ...
  logWithTs("getAvailableMonthKeys: loaded month options", {
    householdId,
    count: sorted.length,
    topMonth: sorted[0] || null,
  });
