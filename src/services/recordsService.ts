...
function normalizeFixedTemplate(
  ...
  return {
    id,
    category: firstString(data.category, data.categoryName, data.name, data.title, "אחר"),
...
export async function saveFixedTemplate(input: FixedTemplateInput): Promise<string> {
  ...
  if (!category) throw new Error("קטגוריה חסרה — יש לבחור קטגוריה לפני שמירה.");
  if (!startDate) throw new Error("תאריך התחלה חסר — יש להזין תאריך תחילת חיוב.");
  if (!Number.isFinite(amount) || amount <= 0) throw new Error("סכום לא תקין — יש להזין סכום חיובי.");
